...
#ci-profiler-menu a:link,#ci-profiler-menu a:visited {
...
#ci-profiler-menu a:hover,#ci-profiler-menu a.current {
...
#ci-profiler-menu-time span,#ci-profiler-benchmarks h2 {
	color: #B72F09;
}

#ci-profiler-menu-memory span,#ci-profiler-memory h2 {
	color: #953FA1;
}

#ci-profiler-menu-queries span,#ci-profiler-queries h2 {
	color: #3769A0;
}

#ci-profiler-menu-vars span,#ci-profiler-vars h2 {
	color: #D28C00;
}

#ci-profiler-menu-files span,#ci-profiler-files h2 {
	color: #5a8616;
}

#ci-profiler-menu-console span,#ci-profiler-console h2 {
	color: #5a8616;
}
...
#ci-profiler-queries b {
	color: white;
}

.ci-profiler-duplicate {
	background: #36363f;
	padding: 4px 0;
}

.ci-profiler-db-explain {
	display: block;
	color: #999;
}

.ci-profiler-db-explain em {
	font-style: normal;
	color: #fffd70;
}
...
<a href="#" id="ci-profiler-menu-open" class="bottom-left"
	onclick="ci_profiler_bar.open(); return false;" style="width: 2em">&nbsp;</a>

<div
	id="codeigniter-profiler" class="bottom-left">
	...
	<div id="ci-profiler-menu">
		...
		<a href="#" id="ci-profiler-menu-console"
			...
			<span><?php echo is_array($sections['console']) ? $sections['console']['log_count'] + $sections['console']['memory_count'] : 0 ?>
		</span> Console
		</a>
		<?php endif; ?>
		...
		<a href="#" id="ci-profiler-menu-time"
			...
			<?php if ($cip_time_format == 'ms') :?> <span><?php echo $this->benchmark->elapsed_time('total_execution_time_start', 'total_execution_time_end') * 1000 ?>
				ms</span> Load Time <?php else: ?> <span><?php echo $this->benchmark->elapsed_time('total_execution_time_start', 'total_execution_time_end') ?>
				s</span> Load Time <?php endif; ?>
		</a> <a href="#" id="ci-profiler-menu-memory"
			onclick="ci_profiler_bar.show('ci-profiler-memory', 'ci-profiler-menu-memory'); return false;">
			<span><?php echo (! function_exists('memory_get_usage')) ? '0' : round(memory_get_usage()/1024/1024, 2).' MB' ?>
		</span> Memory Used
		</a>
		<?php endif; ?>
		...
		<a href="#" id="ci-profiler-menu-files"
			...
			<span><?php echo is_array($sections['files']) ? count($sections['files']) : 0 ?>
		</span> Files
		</a>
		<?php endif; ?>

		<a href="#" id="ci-profiler-menu-exit"
			onclick="ci_profiler_bar.close(); return false;"
	...
	<div id="ci-profiler-console" class="ci-profiler-box"
		...
		<table class="main">
			...
			<tr>
				<td><?php echo $log['type'] ?></td>
				<td><em><?php echo $log['data_type'] ?> </em>: <?php echo $log['name']; ?>
				</td>
	...
	<div id="ci-profiler-vars" class="ci-profiler-box"
		...
		<a href="#"
			onclick="ci_profiler_bar.toggle_data_table('userdata'); return false;">
		...
		<a href="#"
			onclick="ci_profiler_bar.toggle_data_table('<?php echo $section ?>'); return false;">
			<h2>
	...
	<div id="ci-profiler-files" class="ci-profiler-box"
		...
		<table class="main">
			...
			<tr>
				<td class="hilight"><?php echo preg_replace("/\/.*\//", "", $val) ?>
					<br /> <span class="faded small"><?php echo str_replace(FCPATH, '', $val) ?>
				</span>
				</td>
			</tr>
			<?php endforeach; ?>
