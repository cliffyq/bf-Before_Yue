<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	class company_company extends Admin_Controller {
		...
		*/
		...
		public function video_charts($sort_option = "viewcount", $per_page = 5, $offset = 0)
		{
			$this->load->library('pagination');
			$config['base_url'] = base_url() . 'company/company_company/video_charts/'.$sort_option.'/'.$per_page.'/';
			...
			$this->load->model('video/video_model', null, true);
			$video_cards = $this->video_model->video_chart($sort_option, $config['per_page'], $this->uri->segment($config['uri_segment']));
			...
			if($video_cards !== false){
				foreach($video_cards['rows'] as $key=>$video_card)
				//console::log($video_card);
				{
					$videos[$key]=$this->load->module('video')->video_card($video_card);
				}
